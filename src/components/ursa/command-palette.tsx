...
const QUICK_ACTIONS: CommandItem[] = [
  { id: "print", label: "Print / Save as PDF", group: "Action", hint: "Current page", keywords: "pdf export save print", action: () => window.print() },
  { id: "dossier", label: "Open static HTML dossiers", group: "Action", hint: "New tab", keywords: "html reference static original", action: () => window.open("/dossier/index.html", "_blank") },
];
...
export function CommandPalette() {
  ...
  // Build command list from routes
  const commands = useMemo<CommandItem[]>(() => {
    const navItems: CommandItem[] = Object.entries(ROUTES).map(([key, val]) => ({
      ...
      hint: `#/${key}`,
      keywords: key + " " + val.label + " " + val.icon,
      route: key,
    }));
  ...
  if (!open) {
    return (
      ...
      >
        <Search size={15} className="text-ursa-gold group-hover:scale-110 transition" />
        <span className="hidden sm:inline">Quick jump</span>
        <kbd className="hidden sm:inline-flex items-center gap-0.5 text-[0.6rem] bg-ursa-espresso/60 border border-ursa-gold/30 rounded px-1.5 py-0.5 text-ursa-gold-soft">
          ⌘K
        </kbd>
      </button>
    );
    ...
    >
      ...
      <div className="relative w-full max-w-[560px] bg-card border border-ursa-gold/50 rounded-2xl shadow-2xl overflow-hidden ursa-fade-up">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-ursa-line-soft bg-ursa-foam">
          <Search size={18} className="text-ursa-gold shrink-0" />
          <input
            ref={inputRef}
            type="text"
            value={query}
            ...
            onKeyDown={onKeyDown}
            placeholder="Jump to a view, tool, or action…"
        ...
        <div ref={listRef} className="max-h-[52vh] overflow-y-auto ursa-scroll py-2">
          {flatFiltered.length === 0 && (
            <div className="px-6 py-10 text-center">
              ...
              <p className="font-display text-lg text-ursa-dark-roast m-0">No matches</p>
              <p className="text-[0.85rem] text-muted-foreground m-0 mt-1">Try “calculator”, “brand”, or “print”.</p>
            </div>
          )}
          {Object.entries(grouped).map(([group, items]) => {
            ...
            return (
              <div key={group} className="mb-1">
                <div className="px-4 py-1.5 font-label text-[0.6rem] tracking-[0.18em] uppercase text-ursa-gold">
                  {group}
                </div>
                {items.map((item) => {
                  runningIdx++;
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {item.route !== undefined ? <Hash size={15} /> : <CornerDownLeft size={15} />}
                      </span>
                      ...
                      {isActive && (
                        <kbd className="shrink-0 inline-flex items-center gap-0.5 text-[0.58rem] bg-ursa-dark-roast text-ursa-gold-soft rounded px-1.5 py-0.5 border border-ursa-gold/30">
        ...
        <div className="flex items-center justify-between px-4 py-2 border-t border-ursa-line-soft bg-ursa-foam text-[0.68rem] text-muted-foreground">
          ...
          <span className="font-label tracking-[0.1em] uppercase">Ursa Command</span>
        </div>
      </div>
    </div>
  );
